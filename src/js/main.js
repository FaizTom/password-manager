...
readTextFile("../website.json", function(text) {
    webscore = JSON.parse(text);
});
...
function showTable(accounts) {
    ...
    for (var index in accounts) {
        ...
        cols.push($("<td>")
            ...
            //#region Irfan
            .append($("<a>")
                .attr('class', 'ratingScore')
                .data('id', accounts[index].index)
                .on('click', { "index": accounts[index]['other']['url'] }, function(event) { showRatingScore(event.data.index); })
                .text(" check rating"))
            //#endregion
            .append($('<a>')
                .attr('title', "Edit")
...
function showRatingScore(url) {
    ...
    for (var alya = 0; alya < webscore.length; alya++) {
        if (webscore[alya].website == url) {
            score = webscore[alya].rating;
        }
    }
    var h = $('#h4rating');
    h.html('');
    h.append($('<b>').text("Rating for " + url))
    var s = $('#ratingDetails');
    backend.resetTimeout();
    s.html('');
    s.append($('<b>').text(score))
        .append($('<br/>')).append($('<br/>'));
